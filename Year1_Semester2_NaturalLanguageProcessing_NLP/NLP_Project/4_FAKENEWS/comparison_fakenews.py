...
def parse_args():
    parser = argparse.ArgumentParser(description="Compare Fake News models")
    parser.add_argument("--bert_dir", type=str, default=os.path.join(parent_dir, "outputs", "BERT_FakeNews"))
    parser.add_argument("--sbert_dir", type=str, default=os.path.join(parent_dir, "outputs", "SentenceBERT_FakeNews"))
...
def main():
    ...
    csv_path = os.path.join(args.output_dir, "fakenews_model_comparison.csv")
    ...
    plot_model_comparison(df, metrics_to_plot, args.output_dir, "fakenews_metric_barplots.png")
    ...
    if os.path.exists(bert_probs_file) and os.path.exists(sbert_probs_file) and os.path.exists(labels_file):
        ...
        bert_probs = np.load(bert_probs_file)[:, 1] if len(np.load(bert_probs_file).shape) > 1 else np.load(bert_probs_file)
        sbert_probs = np.load(sbert_probs_file)[:, 1] if len(np.load(sbert_probs_file).shape) > 1 else np.load(sbert_probs_file)
        ...
        plt.title('Fake News ROC Comparison')
        plt.legend(loc="lower right")
        plt.savefig(os.path.join(args.output_dir, "fakenews_roc_curves.png"), bbox_inches='tight')
        plt.close()
