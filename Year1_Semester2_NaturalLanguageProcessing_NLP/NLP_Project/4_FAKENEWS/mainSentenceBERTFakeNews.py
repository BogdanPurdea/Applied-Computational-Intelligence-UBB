...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Sentence-BERT on Fake News dataset")
    parser.add_argument("--data_path", type=str, default=os.path.join(parent_dir, "data", "welfake", "WELFake_Dataset.csv"))
    parser.add_argument("--output_dir", type=str, default=os.path.join(parent_dir, "outputs", "SentenceBERT_FakeNews"))
...
def main():
    ...
    # 1. Load Data
    logger.info("Loading WELFake Data...")
    df, text_col = load_csv(args.data_path, text_col="title", label_col="label", sample_size=args.sample_size, text_col_2="text")
    df['label'] = pd.to_numeric(df['label'], errors='coerce')
    df = df.dropna(subset=['label'])
    df['label'] = df['label'].astype(int)

    train_texts, val_texts, test_texts, train_labels, val_labels, test_labels = split_dataset(df, text_col, 'label')
