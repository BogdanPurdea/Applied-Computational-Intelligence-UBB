...
def parse_args():
    ...
    parser.add_argument("--data_path", type=str, default=os.path.join(parent_dir, "data", "romanian_emotion", "romanian_emotion.csv"))
    parser.add_argument("--output_dir", type=str, default=os.path.join(parent_dir, "outputs", "RoBERTa_RED"))
...
def compute_metrics_wrapper(eval_pred):
    ...
    return compute_classification_metrics(labels, predictions, y_probs=probs, average='weighted', is_multiclass=True)
...
def main():
    ...
    # Load data
    logger.info("Loading Romanian Emotion Data...")
    df, text_col = load_csv(args.data_path, text_col="text", label_col="emotion", sample_size=args.sample_size)
    ...
    save_json(mapping, os.path.join(args.output_dir, "label_mapping.json"))
    
    num_labels = len(label_encoder.classes_)
    class_names = list(label_encoder.classes_)

    train_texts, val_texts, test_texts, train_labels, val_labels, test_labels = split_dataset(df, text_col, 'encoded_label')
    ...
    metrics = compute_classification_metrics(test_labels, y_pred, y_probs=y_probs, average='weighted', is_multiclass=True)
