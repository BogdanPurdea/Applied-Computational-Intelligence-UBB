...
class CollectorAgent(Agent):
    ...
    def _evaluate_zones(self):
        """Utility function: find the best zone based on distance (energy cost)."""
    ...
    def next(self):
        # Deliberative phase: decide on goals based on state and blackboard
        
        # 1. If storage is full or battery is low, goal is barge
        if self.state.current_storage >= self.state.storage_capacity or self.state.battery < self.state.position.distance(self.barge_pos) + 20:
    ...
    def action(self) -> Action:
        # Action selection based on deliberation
