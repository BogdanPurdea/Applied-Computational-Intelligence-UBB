...
class Environment:
    ...
    def _generate_environment(self):
        ...
        self.grid[by][bx].is_obstacle = False
        ...
        while obstacles_placed < Config.OBSTACLE_COUNT:
            ...
            if not self.grid[y][x].is_obstacle and (x, y) != Config.BARGE_LOCATION:
        ...
        while clusters_placed < Config.WASTE_CLUSTERS:
            ...
            if not self.grid[cy][cx].is_obstacle and (cx, cy) != Config.BARGE_LOCATION:
                # Add waste in a small radius around cluster center
                for dx in range(-2, 3):
                    for dy in range(-2, 3):
                        nx, ny = cx + dx, cy + dy
                        if 0 <= nx < self.width and 0 <= ny < self.height:
                            if not self.grid[ny][nx].is_obstacle and (nx, ny) != Config.BARGE_LOCATION:
                                density = random.randint(1, 10)
                                self.grid[ny][nx].density = density
                                # Also add turbidity near waste
                                self.grid[ny][nx].turbidity = random.randint(0, Config.MAX_TURBIDITY)
                clusters_placed += 1
    ...
    def update_cell_density(self, pos: Coordinate, new_density: int):
        if 0 <= pos.x < self.width and 0 <= pos.y < self.height:
            self.grid[pos.y][pos.x].density = new_density
