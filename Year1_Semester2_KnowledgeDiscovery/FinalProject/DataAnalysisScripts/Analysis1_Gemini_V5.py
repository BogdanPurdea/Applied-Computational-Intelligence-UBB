"""
CCM Knowledge Discovery / Data Mining Pipeline  –  Gemini V5
Changes from V3:
...
def remove_leakage_columns(df: pd.DataFrame, task: str, strict_no_resistance: bool = False) -> pd.DataFrame:
    """
    Removes target leakage columns from the feature set.
    """
...
def run_clustering(df: pd.DataFrame, output_dir: str):
    """
    Executes unsupervised clustering using K-Means and DBSCAN algorithms.
    Saves cluster summaries and generates 2D visual projections using PCA.
    """
    ...
    if X_scaled.shape[1] >= 2:
        ...
        scatter_km = plt.scatter(X_pca[:, 0], X_pca[:, 1], c=kmeans_labels, cmap="viridis", s=18, alpha=0.7)
        plt.colorbar(scatter_km, label="K-Means Cluster ID")
        plt.title("K-Means Clustering 2D Representation (PCA Projection)")
        plt.xlabel("Principal Component 1")
        plt.ylabel("Principal Component 2")
        save_current_plot(os.path.join(plot_dir, "clustering_kmeans_2d_scatter.png"))

        plt.figure(figsize=(10, 7))
        scatter_db = plt.scatter(X_pca[:, 0], X_pca[:, 1], c=dbscan_labels, cmap="tab20", s=18, alpha=0.7)
        plt.colorbar(scatter_db, label="DBSCAN Cluster ID (-1 denotes Noise)")
        plt.title("DBSCAN Clustering 2D Representation (PCA Projection)")
        plt.xlabel("Principal Component 1")
        plt.ylabel("Principal Component 2")
        save_current_plot(os.path.join(plot_dir, "clustering_dbscan_2d_scatter.png"))
...
def run_anomaly_detection(df: pd.DataFrame, output_dir: str):
    """
    Executes unsupervised anomaly detection using an Isolation Forest model.
    Saves anomaly metrics and outputs a 2D visualization of anomalies via PCA projection.
    """
    ...
    if X_scaled.shape[1] >= 2:
        ...
        plt.figure(figsize=(10, 7))
        is_anom = anomaly_df["is_anomaly"] == 1
        is_norm = anomaly_df["is_anomaly"] == 0

        plt.scatter(X_pca[is_norm, 0], X_pca[is_norm, 1], c="blue", label="Normal", s=18, alpha=0.5)
        plt.scatter(X_pca[is_anom, 0], X_pca[is_anom, 1], c="red", label="Anomaly", s=18, alpha=0.8)
        plt.title("Isolation Forest Anomaly Detection 2D Representation (PCA Projection)")
        plt.xlabel("Principal Component 1")
        plt.ylabel("Principal Component 2")
        plt.legend()
        save_current_plot(os.path.join(plot_dir, "anomaly_detection_2d_scatter.png"))
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = SCRIPT_DIR / "Analysis1_Outputs_Gemini_V5"
