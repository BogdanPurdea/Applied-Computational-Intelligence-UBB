"""
CCM Knowledge Discovery / Data Mining Pipeline  –  Claude V7_2
Changes from V6:
  - Adapted to new dataset schema:
      * Target column renamed: "RUL" → "calculated_RUL_tons"
      * Classification target renamed: "RUL_class" → "RUL_FCA_BIN"
      * All *_BIN and *_ENCODED columns are excluded from features,
        PCA, clustering, and anomaly detection (they are categorical
        encodings / bin labels derived from the raw features).
      * Raw numeric features are scaled (StandardScaler) before all
        analysis; the scaler is applied inside prepare_unsupervised_matrix
        and the supervised pipelines already use StandardScaler internally.
      * Column-name references updated throughout (EDA scatter candidates,
        groupby keys, report sections).
      * datetime_combined replaces "timestamp" for time-series EDA.
      * New raw numeric columns added to EDA scatter candidates:
          swing_frequency, crystallizer_movement, alloy_speed,
          temperature_measurement1, temperature_measurement2
      * workpiece_slice_geometry (string) replaces the old
        workpiece_slice_geometry_encoded (int).
      * alloy_type and steel_type added as categorical EDA groupby keys.
  - All other sections (regression, classification, decision tree, PCA,
    K-Means, DBSCAN, Isolation Forest, association rules) logic intact.

Expected folder structure:
    script_folder/
    ├── Analysis_Claude_V7_2.py
    ├── Dataset/
    │   └── PreProcessedDataset.csv
    └── Analysis1_Outputs_Claude_V7_2/
        └── plots/

Run:
    python Analysis_Claude_V7_2.py
"""
...
# ============================================================
# --- Updated target column names for new schema ---
...
DBSCAN_EPS_OVERRIDE = None   # set to a positive float to override auto-detect

# Cap on BIN columns passed to Apriori to avoid OOM
MAX_APRIORI_COLUMNS = 20
APRIORI_MAX_ROWS = 5_000
APRIORI_MIN_SUPPORT = 0.15
APRIORI_MAX_LEN    = 3

# Suffix patterns for columns to exclude from analysis
_BIN_ENCODED_SUFFIXES = ("_BIN", "_ENCODED")
...
# ============================================================
# Utility – column filtering
# ============================================================
def is_bin_encoded_col(col: str) -> bool:
    """Return True if this column is a *_BIN or *_ENCODED derived column."""
    return any(col.endswith(suf) for suf in _BIN_ENCODED_SUFFIXES)

def raw_numeric_cols(df: pd.DataFrame, extra_exclude=None) -> list:
    """
    Return numeric columns that are NOT *_BIN / *_ENCODED and NOT in extra_exclude.
    """
    exclude = set(extra_exclude or [])
    return [
        c for c in df.select_dtypes(include=[np.number]).columns
        if not is_bin_encoded_col(c) and c not in exclude
    ]
...
def remove_leakage_columns(df: pd.DataFrame, task: str,
                            strict_no_resistance: bool = False) -> pd.DataFrame:
    """
    Removes target leakage columns for the new schema.

    Always dropped (derived / redundant targets):
        RUL_percentage      – linear function of calculated_RUL_tons
        RUL_FCA_ENCODED     – numeric encoding of RUL_FCA_BIN
        cast_in_row_BIN / cast_in_row_ENCODED (and all *_BIN / *_ENCODED cols)

    Task-specific:
        regression    → drop RUL_FCA_BIN  (classification label)
        classification → drop calculated_RUL_tons  (regression target)

    strict_no_resistance:
        Additionally removes all columns containing "resistance".
    """
    cols_to_drop = []

    # Drop ALL *_BIN and *_ENCODED columns – they are derived encodings
    for col in df.columns:
        if is_bin_encoded_col(col):
            cols_to_drop.append(col)

    # Drop RUL_percentage – it is a direct linear derivative of calculated_RUL_tons
    if "RUL_percentage" in df.columns:
        cols_to_drop.append("RUL_percentage")
...
# ============================================================
def plot_bar_from_series(series: pd.Series, title: str, xlabel: str,
                          ylabel: str, path: str, top_n: int = None):
    s = series.dropna()
...
def plot_histogram(series: pd.Series, title: str, xlabel: str,
                   ylabel: str, path: str, bins: int = 40):
...
def plot_line_from_dataframe(df: pd.DataFrame, x_col: str, y_col: str,
                              title: str, xlabel: str, ylabel: str, path: str):
...
# ============================================================
# EDA plots  (updated for new schema)
# ============================================================
def generate_eda_plots(df: pd.DataFrame, output_dir: str):
    ...
    if TARGET_REGRESSION in df.columns:
        plot_histogram(
            ...
            title="RUL Distribution (calculated_RUL_tons)",
            xlabel="RUL (tons)",
            ylabel="Frequency",
    ...
    if TARGET_CLASSIFICATION in df.columns:
        ...
        plot_bar_from_series(
            class_counts,
            title="RUL Class Distribution (RUL_FCA_BIN)",
            xlabel="Count",
            ylabel="RUL Class",
            path=os.path.join(plot_dir, "eda_rul_class_distribution.png"),
        )

    # --- Average RUL by workpiece geometry ---
    if "workpiece_slice_geometry" in df.columns and TARGET_REGRESSION in df.columns:
        ...
        plot_bar_from_series(
            geom_rul,
            title="Average RUL by Workpiece Slice Geometry",
            xlabel="Average RUL (tons)",
            ylabel="Geometry",
            path=os.path.join(plot_dir, "eda_average_rul_by_geometry.png"),
        )

    # --- Average RUL by steel type ---
    ...
    # --- Top correlations with RUL (raw numeric only, no BIN/ENCODED) ---
    if TARGET_REGRESSION in df.columns:
        num_cols = raw_numeric_cols(df, extra_exclude=[TARGET_REGRESSION, "RUL_percentage"])
        numeric_df = df[num_cols + [TARGET_REGRESSION]]
    ...
    # --- Scatter plots: key raw features vs RUL ---
        ...
        for col in candidate_scatter_cols:
            if col in df.columns:
                plot_scatter(
                    ...
                    path=os.path.join(
                        plot_dir,
                        f"eda_scatter_rul_vs_{safe_filename(col)}.png",
                    ),
                )

    # --- Time-series: daily average RUL ---
    datetime_col = "datetime_combined"
    if datetime_col in df.columns and TARGET_REGRESSION in df.columns:
        temp = df.copy()
        temp[datetime_col] = pd.to_datetime(temp[datetime_col], errors="coerce")
        temp = temp.dropna(subset=[datetime_col, TARGET_REGRESSION]).sort_values(datetime_col)
        if not temp.empty:
            daily = (
                temp.set_index(datetime_col)[TARGET_REGRESSION]
                .resample("D")
                .mean()
                .reset_index()
            )
...
def run_rul_regression(df: pd.DataFrame, output_dir: str,
    ...
    predictions_df.to_csv(
        os.path.join(output_dir, "rul_regression_predictions.csv"), index=False
    )
    ...
    plot_scatter(
        predictions_df["actual_RUL"], predictions_df["predicted_RUL"],
        title="RUL Regression: Actual vs Predicted",
        xlabel="Actual RUL (tons)", ylabel="Predicted RUL (tons)",
...
def run_rul_classification(df: pd.DataFrame, output_dir: str,
    ...
    with open(os.path.join(output_dir, "rul_classification_report.txt"),
    ...
    cm_df.to_csv(os.path.join(output_dir, "rul_classification_confusion_matrix.csv"))
...
def run_decision_tree_rules(df: pd.DataFrame, output_dir: str,
    ...
    # Use raw numeric columns only (no BIN/ENCODED)
    numeric_cols = [c for c in X.select_dtypes(include=[np.number]).columns
                    if not is_bin_encoded_col(c)]
    if len(numeric_cols) == 0:
        print("[WARNING] No numeric columns available for decision tree.")
        return
    ...
    with open(os.path.join(output_dir, "decision_tree_rules.txt"),
              "w", encoding="utf-8") as f:
        f.write(rules)
    ...
    if not importance.empty:
        importance.to_csv(
            os.path.join(output_dir, "decision_tree_rule_feature_importance.csv")
        )
        plot_bar_from_series(
...
def prepare_unsupervised_matrix(df: pd.DataFrame):
    """
    Builds the scaled numeric feature matrix used for clustering, PCA,
    and anomaly detection.

    Exclusions:
      - Target columns (calculated_RUL_tons, RUL_FCA_BIN, RUL_percentage)
      - All *_BIN and *_ENCODED columns
      - Identifier / groupby columns (sleeve, num_crystallizer, num_stream)
      - datetime columns
    """
    exclude_exact = {
        TARGET_REGRESSION,
    ...
    # Drop non-numeric, datetime, excluded identifiers, and BIN/ENCODED cols
    X = df.copy()
    # Drop datetime
    datetime_cols = X.select_dtypes(
        include=["datetime64[ns]", "datetimetz"]
    ...
    drop_cols = [
        ...
        if c in exclude_exact or is_bin_encoded_col(c)
    ]
...
def _get_pca3d(X_scaled: np.ndarray) -> tuple:
...
def _plot_kmeans_silhouette_diagram(X_scaled: np.ndarray, labels: np.ndarray,
    ...
    ax.set_title(f"K-Means Silhouette Diagram  (k={k})",
...
def _plot_kmeans_pca2d(components: np.ndarray, labels: np.ndarray, pca,
    ...
    if rul_values is not None and len(rul_values) == len(components):
        ...
        ax.set_title("K-Means Clusters – PCA 2D coloured by RUL",
                     fontsize=14, fontweight="bold")
        ax.set_xlabel(f"PC1 ({pca.explained_variance_ratio_[0]:.2%} variance)")
        ax.set_ylabel(f"PC2 ({pca.explained_variance_ratio_[1]:.2%} variance)")
        ax.grid(True, linestyle="--", alpha=0.3)
...
def _plot_kmeans_pca3d(components3d: np.ndarray, labels: np.ndarray,
                        k: int, plot_dir: str):
...
def _plot_kmeans_centroid_heatmap(X_scaled: np.ndarray, labels: np.ndarray,
    ...
    ax.set_xticklabels([f[:25] for f in feat_subset], rotation=60,
                        ha="right", fontsize=8)
...
def _plot_kmeans_rul_boxplot(labels: np.ndarray, rul_values, k: int, plot_dir: str):
    ...
    ax.set_title("RUL Distribution per K-Means Cluster",
                 fontsize=14, fontweight="bold")
    ax.set_xlabel("K-Means Cluster")
    ax.set_ylabel("RUL (tons)")
...
def _estimate_dbscan_eps(X_scaled: np.ndarray, min_samples: int,
    ...
    print(f"[INFO]   DBSCAN knee at index {knee_idx}, "
...
def _plot_dbscan_pca2d(components: np.ndarray, labels: np.ndarray,
    ...
    ax.set_xlabel(f"PC1 ({pca.explained_variance_ratio_[0]:.2%} variance)")
    ax.set_ylabel(f"PC2 ({pca.explained_variance_ratio_[1]:.2%} variance)")
    ax.legend(title="DBSCAN label", fontsize=8, loc="best",
    ...
    if n_noise > 0 and n_clusters > 0:
        ...
        save_current_plot(
            os.path.join(plot_dir, "dbscan_noise_highlight_scatter.png")
        )
...
def _plot_dbscan_cluster_sizes(labels: np.ndarray, plot_dir: str):
    ...
    bars = ax.bar(label_names, counts, color=bar_colours,
                  edgecolor="white", linewidth=0.8)
    for bar, cnt in zip(bars, counts):
        ax.text(bar.get_x() + bar.get_width() / 2,
                bar.get_height() + max(counts) * 0.01,
                str(cnt), ha="center", va="bottom", fontsize=9, fontweight="bold")
    ax.set_title("DBSCAN Cluster Sizes (incl. Noise)",
                 fontsize=14, fontweight="bold")
...
def _plot_dbscan_rul_per_cluster(labels: np.ndarray, rul_values, plot_dir: str):
    ...
    bars = ax.bar(label_names, means, color=bar_colours,
                  edgecolor="white", linewidth=0.8)
    ...
    ax.set_title("DBSCAN – Average RUL per Cluster",
                 fontsize=14, fontweight="bold")
    ax.set_xlabel("Cluster")
    ax.set_ylabel("Average RUL (tons)")
    ax.grid(True, axis="y", linestyle="--", alpha=0.4)
...
# ============================================================
# Isolation Forest rich visualisations
# ============================================================
...
def _plot_iforest_score_distribution(scores: np.ndarray, is_anomaly: np.ndarray,
                                      plot_dir: str):
    ...
    ax.axvline(normal_scores.mean(), color="#1565C0", linestyle="--",
               linewidth=1.5, label=f"Normal mean={normal_scores.mean():.3f}")
    ax.axvline(anomaly_scores.mean(), color="#B71C1C", linestyle="--",
               linewidth=1.5, label=f"Anomaly mean={anomaly_scores.mean():.3f}")
    ...
    ax.grid(True, axis="y", linestyle="--", alpha=0.4)
    save_current_plot(
        os.path.join(plot_dir, "iforest_score_distribution_by_class.png")
    )

def _plot_iforest_violin(scores: np.ndarray, is_anomaly: np.ndarray,
                          plot_dir: str):
    ...
    ax.set_title("Isolation Forest – Score Violin Plot",
                 fontsize=14, fontweight="bold")
    ax.set_ylabel("Decision Function Score")
    ax.grid(True, axis="y", linestyle="--", alpha=0.4)
    save_current_plot(os.path.join(plot_dir, "iforest_score_violin.png"))

def _plot_iforest_anomaly_by_rul_class(is_anomaly: np.ndarray, rul_class_values,
                                        plot_dir: str):
    if rul_class_values is None:
        return
    print("[INFO]   Plotting Isolation Forest anomaly rate per RUL_FCA_BIN class...")
    ...
    grouped = (
        tmp.groupby("rul_class")["is_anomaly"]
        .value_counts(normalize=True)
        .unstack(fill_value=0)
    )
    grouped.columns = ["Normal" if c == 0 else "Anomaly" for c in grouped.columns]
    if "Normal"  not in grouped.columns: grouped["Normal"]  = 0
    if "Anomaly" not in grouped.columns: grouped["Anomaly"] = 0
    ...
    ax.grid(True, axis="y", linestyle="--", alpha=0.4)
    save_current_plot(
        os.path.join(plot_dir, "iforest_anomaly_proportion_per_rul_class.png")
    )

def _plot_iforest_rul_vs_score(scores: np.ndarray, rul_values,
                                is_anomaly: np.ndarray, plot_dir: str):
    if rul_values is None:
        return
    ...
    a = is_anomaly[valid]
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.scatter(r[a == 0], s[a == 0], s=12, alpha=0.5,
               color="#42A5F5", label="Normal")
    ax.scatter(r[a == 1], s[a == 1], s=28, alpha=0.85, color="#EF5350",
               marker="^", label="Anomaly",
               edgecolors="darkred", linewidths=0.5)
    ax.set_title("Isolation Forest – RUL vs Anomaly Score",
                 fontsize=14, fontweight="bold")
    ax.set_xlabel("RUL (tons)")
...
def run_clustering(df: pd.DataFrame, output_dir: str):
    print("\n[INFO] Running unsupervised clustering "
          "(K-Means + DBSCAN with rich plots)...")
    plot_dir = create_plot_dir(output_dir)
    ...
    rul_values = (
        df[TARGET_REGRESSION].values.astype(float)
    ...
    _plot_kmeans_pca2d(components2d, kmeans_labels, pca2d,
                       KMEANS_N_CLUSTERS, rul_values, plot_dir)
    _plot_kmeans_pca3d(components3d, kmeans_labels, KMEANS_N_CLUSTERS, plot_dir)
    _plot_kmeans_centroid_heatmap(X_scaled, kmeans_labels,
                                   feature_names, KMEANS_N_CLUSTERS, plot_dir)
    ...
    if rul_values is not None:
        avg_rul = (
            pd.Series(rul_values)
            .groupby(pd.Series(kmeans_labels))
            .mean()
            .sort_values(ascending=False)
        )
        plot_bar_from_series(
            ...
            xlabel="Average RUL (tons)", ylabel="K-Means Cluster",
    ...
    _plot_dbscan_pca2d(components2d, dbscan_labels, pca2d,
                       eps, DBSCAN_MIN_SAMPLES, plot_dir)
    ...
    plot_bar_from_series(
        ...
        xlabel="Count", ylabel="DBSCAN Cluster",
...
def run_anomaly_detection(df: pd.DataFrame, output_dir: str):
    print("\n[INFO] Running anomaly detection "
          "(Isolation Forest with rich plots)...")
    plot_dir = create_plot_dir(output_dir)
    ...
    iso = IsolationForest(
        n_estimators=300, contamination=0.05, random_state=RANDOM_STATE,
    )
    ...
    anomaly_df.to_csv(
        os.path.join(output_dir, "anomaly_detection_results.csv"), index=False
    )
    ...
    plot_histogram(
        ...
        xlabel="Anomaly Score", ylabel="Frequency",
    ...
    if TARGET_REGRESSION in anomaly_df.columns:
        ...
        plot_bar_from_series(
            ...
            xlabel="Average RUL (tons)", ylabel="State",
...
def run_association_rule_mining(df: pd.DataFrame, output_dir: str):
    ...
    # Use *_BIN columns (categorical bin labels) + classification target
    bin_cols = [
        c for c in df.columns
        if c.endswith("_BIN") or c == TARGET_CLASSIFICATION
    ]
    if len(bin_cols) < 2:
    ...
    if len(transactions) > APRIORI_MAX_ROWS:
        transactions = transactions.sample(
            n=APRIORI_MAX_ROWS, random_state=RANDOM_STATE, replace=False,
        )
    ...
    if rules.empty:
        ...
        frequent_itemsets.to_csv(
            os.path.join(output_dir, "frequent_itemsets.csv"), index=False
        )
        return
...
def generate_first_layer_knowledge_report(df: pd.DataFrame, output_dir: str):
    ...
    if TARGET_REGRESSION in df.columns:
        lines.append("RUL Summary (calculated_RUL_tons)")
        lines.append("-" * 70)
        lines.append(str(df[TARGET_REGRESSION].describe()))
        lines.append("")

    if "RUL_percentage" in df.columns:
        lines.append("RUL Percentage Summary")
        lines.append("-" * 70)
        lines.append(str(df["RUL_percentage"].describe()))
        lines.append("")

    if TARGET_CLASSIFICATION in df.columns:
        lines.append("RUL_FCA_BIN Distribution")
        lines.append("-" * 70)
        lines.append(str(df[TARGET_CLASSIFICATION].value_counts(dropna=False)))
        lines.append("")

    if "workpiece_slice_geometry" in df.columns and TARGET_REGRESSION in df.columns:
        lines.append("Average RUL by Workpiece Slice Geometry")
        lines.append("-" * 70)
        lines.append(str(df.groupby("workpiece_slice_geometry")[TARGET_REGRESSION]
    ...
    if "steel_type" in df.columns and TARGET_REGRESSION in df.columns:
        ...
        lines.append("-" * 70)
        lines.append(str(df.groupby("steel_type")[TARGET_REGRESSION]
                         .agg(["count", "mean", "median", "min", "max"])))
        lines.append("")

    if "alloy_type" in df.columns and TARGET_REGRESSION in df.columns:
        lines.append("Average RUL by Alloy Type")
        lines.append("-" * 70)
        lines.append(str(df.groupby("alloy_type")[TARGET_REGRESSION]
                         .agg(["count", "mean", "median", "min", "max"])))
        lines.append("")
    ...
    if "sleeve" in df.columns and TARGET_REGRESSION in df.columns:
        ...
        lines.append("-" * 70)
        sleeve_summary = df.groupby("sleeve")[TARGET_REGRESSION].agg(
            ["count", "mean", "min", "max"]
        )
    ...
    if "num_stream" in df.columns and TARGET_REGRESSION in df.columns:
        ...
        lines.append("-" * 70)
        lines.append(str(df.groupby("num_stream")[TARGET_REGRESSION]
                         .agg(["count", "mean", "median", "min", "max"])))
        lines.append("")
    ...
    if "num_crystallizer" in df.columns and TARGET_REGRESSION in df.columns:
        ...
        lines.append("-" * 70)
        lines.append(str(df.groupby("num_crystallizer")[TARGET_REGRESSION]
                         .agg(["count", "mean", "median", "min", "max"])))
        lines.append("")
    ...
    lines.append("-" * 70)
    lines.append("1. calculated_RUL_tons is the primary regression target.")
    lines.append("2. RUL_FCA_BIN (Healthy/Warning/Critical) is the classification target.")
    lines.append("3. All *_BIN and *_ENCODED columns are excluded from ML features and unsupervised analysis.")
    lines.append("4. Cooling variables (water_consumption, water_temperature_delta) are key degradation indicators.")
    lines.append("5. Resistance (tonn) is strongly correlated with RUL — it is the physical basis of RUL derivation.")
    lines.append("6. Steel chemistry (steel_type, alloy_type) provides context for degradation regimes.")
    lines.append("7. Sleeve, crystallizer, and stream identifiers support equipment-specific analysis.")
    lines.append("8. cast_in_row tracks the cumulative number of casts and is a proxy for sleeve age.")
    lines.append("9. *_BIN columns enable association rule mining for interpretable knowledge extraction.")
    lines.append("10. Clustering can reveal process regimes with different average RUL values.")
    lines.append("11. Isolation Forest can detect abnormal casts with accelerated sleeve degradation.")
    lines.append("12. PCA exposes global separation between Healthy, Warning, and Critical states.")
    lines.append("")
    lines.append("Leakage Warning")
    lines.append("-" * 70)
    lines.append("resistance, tonn is the physical basis of calculated_RUL_tons.")
    lines.append("For a cleaner prognostic experiment, run with strict_no_resistance=True.")
    lines.append("RUL_percentage is a linear function of calculated_RUL_tons and is always excluded from features.")
    lines.append("All *_BIN and *_ENCODED columns are always excluded from ML features.")
    lines.append("")
...
if __name__ == "__main__":
    ...
    if TARGET_REGRESSION in df.columns:
        ...
        print(f"[WARNING] '{TARGET_REGRESSION}' not found. Skipping regression.")
    ...
    if TARGET_CLASSIFICATION in df.columns:
        ...
        print(f"[WARNING] '{TARGET_CLASSIFICATION}' not found. "
