...
os.makedirs("FinalProject/output", exist_ok=True)
os.makedirs("FinalProject/plots", exist_ok=True)

print("Step 1: Loading dataset...")
df = pd.read_csv("FinalProject/Dataset/Dataset.csv")

# Clean column headers
df.columns = df.columns.str.strip().str.replace(r'["\']', '', regex=True)

# Parse time columns
temporal_cols = ['date', 'time_temperature_measurement1', 'time_temperature_measurement2', 'sample_time_continuous_caster']
...
numeric_cols = [c for c in df_clean.columns if c not in temporal_cols and c not in categorical_cols and c not in ['RUL', 'timestamp']]

# Pre-impute numeric columns for correlation and model training
...
df_clean['RUL_class'] = np.select(
    [df_clean['RUL'] <= 150, df_clean['RUL'] <= 400, df_clean['RUL'] <= 800],
    ['Critical', 'Low', 'Medium'],
    default='Healthy'
)
...
# To keep the execution fast and markdown compact, we analyze a core representative subset of 20 columns
# covering process parameters, chemistry, temporal elements, and categorical parameters.
columns_to_analyze = [
    'date', 'steel_type', 'workpiece_slice_geometry', 'steel_temperature_grab1, Celsius deg.',
    'resistance, tonn', 'swing_frequency, amount/minute', 'crystallizer_movement, mm',
    'alloy_speed, meter/minute', 'water_consumption, liter/minute', 'water_temperature_delta, Celsius deg.',
    'water_consumption_secondary_cooling_zone_num1, liter/minute', 'C, %', 'Si, %', 'Mn,%', 'S, %', 'P, %',
...
for col in columns_to_analyze:
    ...
    if col_type == "Numeric":
        ...
        plt.tight_layout()
        plot_path = f"FinalProject/plots/{col.replace(' ', '_').replace(',', '_').replace('/', '_')}.png"
    ...
    md_sec.append("\n### 3. Missing Value Analysis")
    md_sec.append(f"- **Relationship to RUL:** Target RUL mean for missing values of `{col}` is {df_clean[df_clean[col].isnull()]['RUL'].mean():.2f} tons vs {df_clean[df_clean[col].notnull()]['RUL'].mean():.2f} tons when populated.")
    ...
    # 6. Binned Conceptual Analysis & FCA Labels
    if col_type == "Numeric":
        md_sec.append("\n### 6. Binned Conceptual Analysis (FCA Scaling)")
        edges = np.unique(np.percentile(df_imputed[col].dropna(), [0, 33.3, 66.6, 100]))
        if len(edges) > 1:
            bins = pd.cut(df_imputed[col], bins=edges, labels=['Low', 'Medium', 'High'][:len(edges)-1], include_lowest=True)
            binned_rul = df_clean.groupby(bins)['RUL'].mean().to_dict()
            md_sec.append("- **Mean RUL by bin:**")
            for b, v in binned_rul.items():
                md_sec.append(f"  - `{b}`: {v:.2f} tons")
        else:
            md_sec.append("- **Mean RUL by bin:** Only one bin available due to constant value.")
        md_sec.append(f"- **FCA Attribute Labels:** `low_{col.split(',')[0]}`, `medium_{col.split(',')[0]}`, `high_{col.split(',')[0]}`")
        
    # 7. RUL Class Overrepresentation
...
# Setup GroupShuffleSplit on sleeve
...
for name, model in reg_models.items():
    try:
        if name in ['SVR', 'KNN Regressor', 'MLP Regressor']:
            # Slow models, sub-sample train
            sub_idx = np.random.choice(len(X_train_s), min(2000, len(X_train_s)), replace=False)
...
df_reg_metrics.to_csv("FinalProject/output/model_comparison.csv", index=False)
...
df_clf_metrics.to_csv("FinalProject/output/model_comparison_classifier.csv", index=False)
...
df_feat_imp.to_csv("FinalProject/output/feature_importance.csv", index=False)

# 5. Unsupervised Clustering & PCA
print("Step 6: Running Clustering & PCA...")
X_sample = StandardScaler().fit_transform(X.sample(min(1500, len(X)), random_state=42))
...
plt.tight_layout()
plt.savefig("FinalProject/plots/kmeans_operating_regimes.png")
plt.close()

# 6. Association Rules
print("Step 7: Mining Association Rules...")
df_assoc = pd.DataFrame()
df_assoc['High_Temp'] = df_clean['steel_temperature_grab1, Celsius deg.'] >= df_clean['steel_temperature_grab1, Celsius deg.'].median()
df_assoc['High_Resistance'] = df_clean['resistance, tonn'] >= df_clean['resistance, tonn'].median()
df_assoc['High_Cooling'] = df_clean['water_consumption, liter/minute'] >= df_clean['water_consumption, liter/minute'].median()
df_assoc['Critical_RUL'] = df_clean['RUL'] <= 150

freq_items = apriori(df_assoc, min_support=0.01, use_colnames=True)
rules = association_rules(freq_items, metric="confidence", min_threshold=0.3)
rules.to_csv("FinalProject/output/association_rules.csv", index=False)
...
# Write reports
md_report_path = "FinalProject/output/column_analysis_report.md"
with open(md_report_path, "w", encoding="utf-8") as f:
    f.write("\n".join(report_content))

html_report_path = "FinalProject/output/column_analysis_report.html"
# Simple conversion wrapper for basic HTML visualization of markdown
...
print("  - FinalProject/output/model_comparison.csv")
print("  - FinalProject/output/feature_importance.csv")
print("  - FinalProject/output/association_rules.csv")
